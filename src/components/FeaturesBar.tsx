...
export default function FeaturesBar() {
  const features = [
    ...
    },
    {
      ...
      desc: "Produits premium certifiés",
    },
    {
      icon: (
        <path
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 py-12 sm:px-6 md:grid-cols-4 lg:px-8">
        {features.map((f) => (
          <div key={f.title} className="flex flex-col items-center text-center">
            <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-white shadow-card">
              <svg
            ...
            <p className="mt-1 text-xs text-ink-faint">{f.desc}</p>
          </div>
        ))}
      </div>
    </section>
