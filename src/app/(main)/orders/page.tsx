...
function OrdersContent() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
      <h1 className="text-3xl font-bold text-ink dark:text-white">طلباتي</h1>
      ...
      <form onSubmit={search} className="mt-8 space-y-4 rounded-2xl border border-neutral-100 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
        <input
          placeholder="رقم الطلب (مثال: SM202606-1234)"
          value={orderNumber}
          onChange={(e) => setOrderNumber(e.target.value)}
          className="w-full rounded-xl border border-neutral-200 px-4 py-3 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-white"
          dir="ltr"
        />
        ...
        <input
          ...
          dir="ltr"
        />
        <button type="submit" className="w-full rounded-xl bg-ink py-3 text-sm font-bold text-white dark:bg-whatsapp">
          بحث
        </button>
      </form>
      ...
      <div className="mt-8 space-y-4">
        {displayOrders.map((order) => (
          <div key={order.id} className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-card dark:border-neutral-800 dark:bg-neutral-900">
            ...
            <ul className="mt-4 space-y-1 border-t border-neutral-100 pt-4 dark:border-neutral-800">
            ...
            <a
              href={buildWhatsAppContactUrl(`السلام عليكم، بخصوص الطلب ${order.orderNumber}`)}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-4 inline-flex text-sm font-bold text-whatsapp"
            >
              تواصل عبر واتساب
            </a>
          </div>
